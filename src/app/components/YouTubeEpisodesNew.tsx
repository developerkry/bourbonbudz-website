  ...
  useEffect(() => {
    const fetchVideos = async () => {
      try {
        ...
        if (!apiKey || !channelId) {
          throw new Error('YouTube API credentials not found. Please set NEXT_PUBLIC_YOUTUBE_API_KEY and NEXT_PUBLIC_YOUTUBE_CHANNEL_ID in your environment variables.');
        }

        const fetchedVideos = await fetchChannelVideos(channelId, apiKey, 20);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {videos.map((video) => (
        <a
          ...
          className="bg-zinc-900 rounded-lg overflow-hidden border border-zinc-800 hover:border-amber-500/30 transition-all duration-300 group"
        >
          <div className="aspect-video bg-gradient-to-br from-amber-900/20 to-zinc-800 flex items-center justify-center relative overflow-hidden">
            <img
              src={video.thumbnail}
              alt={video.title}
